...
def serialize_policy(row: AlertPolicy) -> dict:
    return {
        ...
        "trigger_states": json.loads(row.trigger_states_json or "[]"),
        "escalation_minutes": json.loads(row.escalation_minutes_json or "[]"),
...
async def upsert_policy(db: AsyncSession, policy_key: str, body) -> AlertPolicy:
    ...
    values = {
        "service_key": body.service_key.upper() if body.service_key else None,
        "minimum_severity": body.minimum_severity.lower(),
        "trigger_states_json": json.dumps(body.trigger_states),
        "escalation_minutes_json": json.dumps(body.escalation_minutes),
        "targets_json": json.dumps(body.targets),
        "enabled": body.enabled,
    }
    if row is None:
        row = AlertPolicy(policy_key=key, **values)
        db.add(row)
    else:
...
async def list_alerts(db: AsyncSession, status_filter: str | None = None, service_key: str | None = None, limit: int = 100) -> list[Alert]:
    ...
    if service_key:
        stmt = stmt.where(Alert.service_key == service_key.upper())
...
async def evaluate_signal(db: AsyncSession, service_key: str, state: str, severity: str, details: dict, incident_id: str | None = None) -> list[Alert]:
    ...
    for policy in policies:
        if not policy.enabled or (policy.service_key and policy.service_key != service_key):
            continue
        if state not in json.loads(policy.trigger_states_json or "[]"):
            continue
        if SEVERITY_RANK.get(severity, 0) < SEVERITY_RANK.get(policy.minimum_severity, 1):
            continue
        ...
        if row is None:
            row = Alert(dedupe_key=dedupe, policy_key=policy.policy_key, service_key=service_key, severity=severity, state=state, incident_id=incident_id, details_json=json.dumps(details))
            db.add(row)
...
async def advance_escalations(db: AsyncSession) -> list[Alert]:
    open_alerts = list((await db.execute(select(Alert).where(Alert.status == "open"))).scalars().all())
    policies = {p.policy_key: p for p in await list_policies(db)}
    changed: list[Alert] = []
    now = _now()
    for alert in open_alerts:
        ...
        if not policy:
            continue
        stages = json.loads(policy.escalation_minutes_json or "[]")
        target_stage = sum(1 for minute in stages if now >= alert.opened_at.replace(tzinfo=alert.opened_at.tzinfo or timezone.utc) + timedelta(minutes=minute))
        if target_stage > alert.escalation_stage:
            alert.escalation_stage = target_stage
...
async def alert_summary(db: AsyncSession) -> dict:
    rows = await list_alerts(db, limit=500)
    active = [r for r in rows if r.status != "resolved"]
    return {
        "active_total": len(active),
        "open": sum(r.status == "open" for r in active),
        "acknowledged": sum(r.status == "acknowledged" for r in active),
        "critical": sum(r.severity == "critical" for r in active),
        "escalated": sum(r.escalation_stage > 0 for r in active),
    }
