...
async def configuration_rollback(
    scope: str,
    config_key: str,
    version: int,
    db: AsyncSession = Depends(get_db),
...
<div class='top'><div><div class='brand'>UNG-CORE Control Center</div><div class='sub'>National Grid ecosystem operations and dependency view</div></div><div class='toolbar'><input id='token' type='password' placeholder='Bearer token'><button onclick='loadAll()'>Connect</button></div></div>
<div class='grid'><div class='card'><div class='sub'>Systems</div><div id='total' class='metric'>—</div></div><div class='card'><div class='sub'>Healthy</div><div id='healthy' class='metric'>—</div></div><div class='card'><div class='sub'>Degraded / Unknown</div><div id='degraded' class='metric'>—</div></div><div class='card'><div class='sub'>Unavailable</div><div id='down' class='metric'>—</div></div></div>
...
async function loadAll(){try{const [s,t]=await Promise.all([get('/v1/control-center/status'),get('/v1/control-center/topology')]);
document.getElementById('total').textContent=s.total_systems;document.getElementById('healthy').textContent=s.healthy;document.getElementById('degraded').textContent=s.degraded_or_unknown;document.getElementById('down').textContent=s.unavailable;
