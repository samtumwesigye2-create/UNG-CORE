...
async def operations(db: AsyncSession = Depends(get_db), _: Principal = Depends(require_permission("ung.core.control.read"))):
    return {
        ...
        "approvals": await approval_summary(db),
        "jobs": await job_summary(db),
...
async def configuration_rollback(scope: str, config_key: str, version: int, approval_request_id: str | None = None, db: AsyncSession = Depends(get_db), principal: Principal = Depends(require_permission("ung.core.config.write"))):
    gate = await assert_gate(
        db,
        action="config.rollback",
        actor_id=principal.subject,
        target_type="configuration",
        target_id=f"{scope}:{config_key}:{version}",
        context={"scope": scope, "config_key": config_key, "version": version},
...
async def control_center_ui():
    return """<!doctype html><html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>UNG-CORE Control Center</title><style>:root{font-family:Inter,system-ui,sans-serif;background:#0b1020;color:#eef2ff}body{margin:0}.shell{max-width:1180px;margin:auto;padding:28px}.top{display:flex;justify-content:space-between;gap:16px;align-items:center}.brand{font-size:28px;font-weight:800}.sub{color:#9aa6c1}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:24px 0}.card,.panel{background:#131a2e;border:1px solid #26304b;border-radius:16px;padding:18px}.metric{font-size:30px;font-weight:800}.panel{margin-top:14px}.systems{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.system{background:#0e1528;border:1px solid #26304b;border-radius:12px;padding:14px}.badge{display:inline-block;padding:4px 9px;border-radius:999px;background:#26304b;font-size:12px}.toolbar{display:flex;gap:8px;flex-wrap:wrap}input,button{border-radius:9px;border:1px solid #34405f;background:#0e1528;color:#eef2ff;padding:10px 12px}button{cursor:pointer;font-weight:700}@media(max-width:800px){.grid,.systems{grid-template-columns:1fr 1fr}}@media(max-width:520px){.grid,.systems{grid-template-columns:1fr}.top{align-items:flex-start;flex-direction:column}}</style></head><body><div class='shell'><div class='top'><div><div class='brand'>UNG-CORE Control Center</div><div class='sub'>National Grid operations, approvals, jobs, cross-system execution and event delivery</div></div><div class='toolbar'><input id='token' type='password' placeholder='Bearer token'><button onclick='loadAll()'>Connect</button></div></div><div class='grid'><div class='card'><div class='sub'>Systems</div><div id='total' class='metric'>—</div></div><div class='card'><div class='sub'>Healthy Services</div><div id='healthy' class='metric'>—</div></div><div class='card'><div class='sub'>Queued Jobs</div><div id='jobs' class='metric'>—</div></div><div class='card'><div class='sub'>Event Failures</div><div id='events' class='metric'>—</div></div></div><div class='panel'><h3>System Status</h3><div id='systems' class='systems'><span class='sub'>Connect to load status.</span></div></div><div class='panel'><h3>Operational State</h3><div id='ops' class='sub'>Connect to load state.</div></div><div class='panel'><h3>Ecosystem Topology</h3><div id='topology' class='sub'>Connect to load dependency graph.</div></div></div><script>const headers=()=>({Authorization:'Bearer '+document.getElementById('token').value});async function get(path){const r=await fetch(path,{headers:headers()});if(!r.ok)throw new Error(await r.text());return r.json()}async function loadAll(){try{const [s,t,o]=await Promise.all([get('/v1/control-center/status'),get('/v1/control-center/topology'),get('/v1/control-center/operations')]);document.getElementById('total').textContent=s.total_systems;document.getElementById('healthy').textContent=o.telemetry.healthy;document.getElementById('jobs').textContent=o.jobs.queued+o.jobs.scheduled+o.jobs.retry_wait;document.getElementById('events').textContent=o.event_delivery.failed;document.getElementById('systems').innerHTML=s.systems.map(x=>`<div class='system'><b>${x.display_name}</b><div class='sub'>${x.system_key}</div><p><span class='badge'>${x.status}</span> <span class='badge'>${x.criticality}</span></p></div>`).join('')||'<span class="sub">No systems registered.</span>';document.getElementById('ops').innerHTML=`Jobs: <b>${o.jobs.queued}</b> queued · <b>${o.jobs.running}</b> running · <b>${o.jobs.failed}</b> failed<br>Events: <b>${o.event_delivery.delivered}</b> delivered · <b>${o.event_delivery.retry_wait}</b> retrying · <b>${o.event_delivery.failed}</b> failed<br>Approvals: <b>${o.approvals.pending}</b> pending · Alerts: <b>${o.alerts.active_total}</b> active · Incidents: <b>${o.incidents.open_total}</b> open<br>Telemetry: <b>${o.telemetry.healthy}</b> healthy · <b>${o.telemetry.degraded}</b> degraded · <b>${o.telemetry.offline}</b> offline`;document.getElementById('topology').innerHTML=`<b>${t.node_count}</b> systems · <b>${t.edge_count}</b> dependencies<br><br>`+t.edges.map(e=>`${e.source} → ${e.target} <span class='badge'>${e.type}${e.required?' · required':''}</span>`).join('<br>')}catch(e){alert('Control Center: '+e.message)}}</script></body></html>"""
