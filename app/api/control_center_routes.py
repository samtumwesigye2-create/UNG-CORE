...
<div class='top'><div><div class='brand'>UNG-CORE Control Center</div><div class='sub'>National Grid ecosystem operations, approvals, routing, telemetry, alerts, audit and command history</div></div><div class='toolbar'><input id='token' type='password' placeholder='Bearer token'><button onclick='loadAll()'>Connect</button></div></div>
...
<div class='panel'><h3>Operational State</h3><div id='ops' class='sub'>Connect to load routing, incident, telemetry, alert, approval, audit and command state.</div></div>
...
document.getElementById('total').textContent=s.total_systems;document.getElementById('healthy').textContent=o.telemetry.healthy;document.getElementById('alerts').textContent=o.alerts.active_total;document.getElementById('approvals').textContent=o.approvals.pending;
...
document.getElementById('ops').innerHTML=`Gateway: <b>${o.gateway.enabled_routes}</b> enabled / <b>${o.gateway.total_routes}</b> total routes<br>Incidents: <b>${o.incidents.open_total}</b> open · <b>${o.incidents.critical}</b> critical<br>Alerts: <b>${o.alerts.active_total}</b> active · <b>${o.alerts.critical}</b> critical · <b>${o.alerts.escalated}</b> escalated<br>Approvals: <b>${o.approvals.pending}</b> pending · <b>${o.approvals.approved}</b> approved · <b>${o.approvals.denied}</b> denied · <b>${o.approvals.expired}</b> expired<br>Commands: <b>${o.commands.total_recent}</b> recent · <b>${o.commands.succeeded}</b> succeeded · <b>${o.commands.failed}</b> failed<br>Audit: <b>${o.audit.total_recent}</b> recent events<br>Telemetry: <b>${o.telemetry.healthy}</b> healthy · <b>${o.telemetry.degraded}</b> degraded · <b>${o.telemetry.offline}</b> offline · <b>${o.telemetry.unknown}</b> unknown`;
