...
from app.api.ml_drift_routes import router as ml_drift_router
from app.api.ml_auto_retraining_routes import router as ml_auto_retraining_router
...
from app.api.ml_serving_routes import router as ml_serving_router
from app.api.ml_health_routes import router as ml_health_router
from app.api.ml_alert_routes import router as ml_alert_router
from app.api.ml_feedback_routes import router as ml_feedback_router
from app.api.ml_lifecycle_routes import router as ml_lifecycle_router
from app.api.ml_calibration_routes import router as ml_calibration_router
from app.api.ml_multifeature_routes import router as ml_multifeature_router
...
def _host_allowed(host: str) -> bool:
    if not trusted_hosts or trusted_hosts == ["*"]: return True
    hostname = host.split(":", 1)[0].lower(); return any(fnmatch(hostname, pattern) for pattern in trusted_hosts)

@app.middleware("http")
async def hardening_middleware(request: Request, call_next):
    if request.url.path not in {"/health", "/ready"}:
        host = request.headers.get("host", "")
        if not _host_allowed(host): return JSONResponse(status_code=400, content={"detail": "invalid host header"})
    content_length = request.headers.get("content-length")
    if content_length and int(content_length) > settings.request_body_limit_bytes: return JSONResponse(status_code=413, content={"detail": "request body too large"})
    try: response = await asyncio.wait_for(call_next(request), timeout=settings.request_timeout_seconds)
    except asyncio.TimeoutError: return JSONResponse(status_code=504, content={"detail": "request timed out"})
    if settings.security_headers_enabled:
        response.headers["X-Content-Type-Options"] = "nosniff"; response.headers["X-Frame-Options"] = "DENY"; response.headers["Referrer-Policy"] = "no-referrer"; response.headers["Cache-Control"] = "no-store"; response.headers["Permissions-Policy"] = "camera=(), microphone=(), geolocation=()"
...
app.include_router(router); app.include_router(ml_router); app.include_router(ml_model_registry_router); app.include_router(ml_evaluation_router); app.include_router(ml_training_pipeline_router); app.include_router(ml_drift_router); app.include_router(ml_auto_retraining_router); app.include_router(ml_explainability_router); app.include_router(ml_prediction_router); app.include_router(ml_tuning_router); app.include_router(ml_ensemble_router); app.include_router(ml_serving_router); app.include_router(ml_health_router); app.include_router(ml_alert_router); app.include_router(ml_feedback_router); app.include_router(ml_lifecycle_router); app.include_router(ml_calibration_router); app.include_router(ml_multifeature_router); app.include_router(auth_router); app.include_router(control_center_router); app.include_router(operator_router); app.include_router(gateway_router); app.include_router(routing_router); app.include_router(telemetry_router); app.include_router(alerting_router); app.include_router(audit_command_router); app.include_router(approval_router); app.include_router(scheduler_router); app.include_router(event_delivery_router); app.include_router(recovery_router); app.include_router(resource_router); app.include_router(unified_operations_router)
