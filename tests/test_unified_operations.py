...
from app.services.unified_operations import InvalidOperation, evaluate_transition, parse_semver
...
def test_operator_command_parser_rejects_unknown_command():
    with pytest.raises(CommandError): parse_command("UNG DESTROY EVERYTHING")

def test_semver_parser_is_strict():
    assert parse_semver("2.4.1")== (2,4,1)
    with pytest.raises(InvalidOperation): parse_semver("v2.4")
